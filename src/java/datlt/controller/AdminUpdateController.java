...
public class AdminUpdateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int bookId = Integer.parseInt(request.getParameter("updateId"));
            BookDAO dao = new BookDAO();
            BookDTO book = dao.findByPrimaryKey(bookId);
            ...
            request.setAttribute("images", book.getImage());
            request.setAttribute("description", book.getDescription());

        } catch (Exception e) {
            log("ERROR at AdminUpdateController: " + e.getMessage());
        } finally {
            request.getRequestDispatcher("adminUpdateBook.jsp").forward(request, response);
        }
    }
